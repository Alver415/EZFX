...
public class IntrospectingBeanEditor<T> extends BeanEditor<T> {
	...
	public IntrospectingBeanEditor(T target) {
...
//				.sorted(Comparator.comparingInt(PropertyInfo::order).thenComparing(PropertyInfo::displayName))
				.map(propertyInfo -> _buildEditorControl(target, propertyInfo)).toList();
	}

	@SuppressWarnings("unchecked")
	private <S> Editor<?, ?> _buildEditorControl(Object target, PropertyInfo<?> propertyInfo) {
		return buildEditorControl((S) target, (PropertyInfo<S>) propertyInfo);
	}

	private <S> Editor<S, ?> buildEditorControl(S target, PropertyInfo<S> propertyInfo) {
		EditorControl<S> control = getEditorControlFactory().buildEditorControl(
