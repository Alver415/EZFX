...
public class SpringFXLoader {
	...
	/**
	 * Delimiter for default values defined in bidirectional mappings.
	 */
	...
	/**
	 * A key for ApplicationContext in namespace map.
	 *
	 * @see #getNamespace()
	 * @since JavaFX 2.2
	 */
	...
	private final LinkedList<SpringFXLoader> loaders;
	private final ControllerAccessor controllerAccessor = new ControllerAccessor();
	private ApplicationContext applicationContext;
	private URL location;
	private ResourceBundle resources;
	private final ObservableMap<String, Object> namespace = FXCollections.observableHashMap();
	...
	private SpringFXLoader parentLoader;
	...
	/**
	 * Creates a new SpringFXLoader instance.
	 */
	public SpringFXLoader() {
		this((URL) null);
	}
	 ...
	 * @since JavaFX 2.1
	 */
	public SpringFXLoader(URL location) {
		this(location, null);
	}
	 ...
	 * @since JavaFX 2.1
	 */
	public SpringFXLoader(URL location, ResourceBundle resources) {
	 ...
	 * @since JavaFX 2.1
	 */
	public SpringFXLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory) {
	...
	public SpringFXLoader(
			...
			Callback<Class<?>, Object> controllerFactory,
			Charset charset,
			LinkedList<SpringFXLoader> loaders) {
		this(location, resources, builderFactory, controllerFactory, charset, loaders, null);
	}
	...
	public static void setDefaultClassLoader(ClassLoader defaultClassLoader) {
		SpringFXLoader.defaultClassLoader = Objects.requireNonNull(defaultClassLoader);
	}
	...
	static int compareJFXVersions(String rtVer, String nsVer) {
		...
		if (rtVer == null || rtVer.isEmpty() || nsVer == null || nsVer.isEmpty()) {
			return retVal;
		}
	...
	public void setApplicationContext(ApplicationContext applicationContext) {
		...
		if (this.controllerFactory == null && applicationContext != null) {
			this.controllerFactory = applicationContext::getBean;
		}
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof SpringFXLoader loader) {
	...
	private boolean isCyclic(
			SpringFXLoader currentLoader, SpringFXLoader node) {
		...
		try {
			...
			if (controller != null) {
				if (controller instanceof Initializable) {
					...
					Method initializeMethod = controllerAccessor.getControllerMethods()
	...
	private String constructFXMLTrace() {
		...
		for (SpringFXLoader loader : loaders) {
	...
	private void processProcessingInstruction() throws LoadException {
		...
		switch (piTarget) {
			case LANGUAGE_PROCESSING_INSTRUCTION -> processLanguage();
			case IMPORT_PROCESSING_INSTRUCTION -> processImport();
			case COMPILE_PROCESSING_INSTRUCTION -> {
				String strCompile = xmlStreamReader.getPIData().trim();
				// if PIData() is empty string then default to true, otherwise use Boolean.parseBoolean(string) to
				// determine the boolean value
				compileScript = (strCompile.isEmpty() || Boolean.parseBoolean(strCompile));
			}
		}
	}
	...
	private void createElement() throws IOException {
		...
		if (prefix == null) {
			...
			switch (localName) {
				...
				default -> throw constructLoadException(prefix + ":" + localName + " is not a valid element.");
			}
		} else {
	...
	private enum SupportedType {
		PARAMETERLESS {
			...
			@Override
			protected boolean methodIsOfType(Method m) {
				return m.getParameterTypes().length == 1 && m.getParameterTypes()[0].equals(
			...
			@Override
			protected boolean methodIsOfType(Method m) {
				return m.getParameterTypes().length == 3 && ObservableValue.class.isAssignableFrom(
						m.getParameterTypes()[0]) && m.getParameterTypes()[1].equals(m.getParameterTypes()[2]);
			}
	...
	private record Attribute(String name, Class<?> sourceType, String value) {
	}

	// Event handler that delegates to a method defined by the controller object
	private record ControllerMethodEventHandler<T extends Event>(MethodHandler handler) implements EventHandler<T> {
	...
	private static class ScriptEventHandler implements EventHandler<Event> {
		...
		public ScriptEventHandler(String script, ScriptEngine scriptEngine, String filename) {
			...
			if (scriptEngine instanceof Compilable && compileScript) {
				try {
					// supply the filename to the scriptEngine engine scope Bindings in case it is needed for
					// compilation
					...
					Logging.getJavaFXLogger().warning(
							filename + ": compiling caused \"" + compileExc + "\", falling back to evaluating script" +
									" " +
									"in uncompiled mode");
				}
			}
	...
	private record ObservableListChangeAdapter(MethodHandler handler) implements ListChangeListener {
	...
	private record ObservableMapChangeAdapter(MethodHandler handler) implements MapChangeListener {
	...
	private record ObservableSetChangeAdapter(MethodHandler handler) implements SetChangeListener {
	...
	private record PropertyChangeAdapter(MethodHandler handler) implements ChangeListener<Object> {
	...
	private record MethodHandler(Object controller, Method method, SupportedType type) {
	...
	private static final class ControllerAccessor {
		...
		private static int getAccess(final int fullModifiers) {
			...
			return switch (untransformedAccess) {
		...
		private void addAccessibleMembers(
				final Class<?> type,
		...
		private void addAccessibleFields(
				final Class<?> type, final int allowedMemberAccess) {
			...
			for (final Field field : fields) {
				...
				if (((memberModifiers & (Modifier.STATIC | Modifier.FINAL)) != 0) || ((getAccess(
						memberModifiers) & allowedMemberAccess) == 0)) {
					continue;
				}
		...
		private void addAccessibleMethods(
				final Class<?> type, final int allowedMemberAccess) {
			final boolean isPublicType = Modifier.isPublic(type.getModifiers());
			...
			for (final Method method : methods) {
				...
				if (((memberModifiers & (Modifier.STATIC | Modifier.NATIVE)) != 0) || ((getAccess(
						memberModifiers) & allowedMemberAccess) == 0)) {
					continue;
				}
	...
	private abstract class Element {
		...
		public void processInstancePropertyAttributes() throws IOException {
			if (!instancePropertyAttributes.isEmpty()) {
		...
		private boolean isBidirectionalBindingExpression(String aValue) {
			return aValue.startsWith(BI_DIRECTIONAL_BINDING_PREFIX) && aValue.endsWith(BI_DIRECTIONAL_BINDING_SUFFIX);
		}
		...
		private Object resolvePrefixedValue(String aValue) throws LoadException {
			if (aValue.startsWith(ESCAPE_PREFIX)) {
				...
				if (aValue.isEmpty() || !(aValue.startsWith(ESCAPE_PREFIX) || aValue.startsWith(
						RELATIVE_PATH_PREFIX) || aValue.startsWith(RESOURCE_KEY_PREFIX) || aValue.startsWith(
						EXPRESSION_PREFIX) || aValue.startsWith(BI_DIRECTIONAL_BINDING_PREFIX))) {
					...
					if (aValue.charAt(0) == '/') {
						...
						if (res == null) {
							throw constructLoadException(
									"Invalid resource: " + aValue + " not found on the " + "classpath");
						}
						return res.toString();
					} else {
						try {
							return new URL(SpringFXLoader.this.location, aValue).toString();
						} catch (MalformedURLException e) {
							System.err.println(SpringFXLoader.this.location + "/" + aValue);
						}
					}
				}
		...
		private Object populateArrayFromString(
				Class<?> type, String stringValue) throws LoadException {
		...
		private void populateListFromString(
				BeanAdapter valueAdapter, String listPropertyName, String stringValue) throws LoadException {
		...
		private <T> T getExpressionObjectOfType(String handlerValue, Class<T> type) throws LoadException {
			...
			if (expression != null) {
				...
				throw constructLoadException(
						"Error resolving \"" + handlerValue + "\" expression." + "Does not point to a " + type.getName());
			}
			return null;
		}

		private MethodHandler getControllerMethodHandle(
				String handlerName, SupportedType... types) throws LoadException {
			if (handlerName.startsWith(CONTROLLER_METHOD_PREFIX)) {
				...
				if (!handlerName.startsWith(CONTROLLER_METHOD_PREFIX)) {
					...
					Method method = controllerAccessor.getControllerMethods().get(SupportedType.PARAMETERLESS).get(
							handlerName);
					if (method != null) {
		...
		public void processEventHandlerAttributes() throws LoadException {
			if (!eventHandlerAttributes.isEmpty() && !staticLoad) {
				for (Attribute attribute : eventHandlerAttributes) {
					...
					if (value instanceof ObservableList && attribute.name.equals(COLLECTION_HANDLER_NAME)) {
						...
						if (eventHandler == null) {
							if (handlerName.isEmpty() || scriptEngine == null) {
								throw constructLoadException(
										"Error resolving " + attribute.name + "='" + attribute.value + "', either " +
												"the" +
												" " +
												"event handler is not in the Namespace or there is " + "an" + " " +
												"error in the script.");
							}
							eventHandler = new ScriptEventHandler(
									handlerName, scriptEngine,
									location.getPath() + "-" + attribute.name + "_attribute_in_element_ending_at_line_"
											+ getLineNumber());
						}
		...
		private void processObservableListHandler(String handlerValue) throws LoadException {
			...
			if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
				...
				if (listener instanceof ListChangeListener) {
					...
					throw constructLoadException(
							"Error resolving \"" + handlerValue + "\" expression." + "Must be either " +
									"ListChangeListener or InvalidationListener");
				}
			}
		}
		...
		private void processPropertyHandler(String attributeName, String handlerValue) throws LoadException {
			...
			if (i != j) {
				...
				if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
					final MethodHandler handler = getControllerMethodHandle(
							handlerValue, SupportedType.PROPERTY_CHANGE_LISTENER, SupportedType.EVENT);
					if (handler != null) {
						if (handler.type == SupportedType.EVENT) {
							// Note: this part is solely for purpose of 2.2 backward compatibility where an Event
							// object
							// has been used instead of usual property change parameters
							propertyModel.addListener((observable, oldValue, newValue) -> handler.invoke(
									new Event(value, null, Event.ANY)));
						} else {
	...
	private abstract class ValueElement extends Element {
		...
		public void processEndElement() throws IOException {
			...
			if (!staticPropertyAttributes.isEmpty()) {
			...
			if (!staticPropertyElements.isEmpty()) {
		...
		private void processValue() throws LoadException {
			...
			if (parent == null) {
				...
				String fxNamespaceURI = xmlStreamReader.getNamespaceContext().getNamespaceURI("fx");
				if (fxNamespaceURI != null) {
					String fxVersion = fxNamespaceURI.substring(fxNamespaceURI.lastIndexOf("/") + 1);
					if (compareJFXVersions(FX_NAMESPACE_VERSION, fxVersion) < 0) {
						throw constructLoadException(
								"Loading FXML document of version " + fxVersion + " by JavaFX runtime supporting " +
										"version " + FX_NAMESPACE_VERSION);
					}
				}
				...
				if (defaultNSURI != null) {
					...
					if (compareJFXVersions(JAVAFX_VERSION, nsVersion) < 0) {
						Logging.getJavaFXLogger().warning(
								"Loading FXML document with JavaFX API of version " + nsVersion + " by JavaFX " +
										"runtime" +
										" " +
										"of version " + JAVAFX_VERSION);
					}
				}
			}
			...
			if (fx_id != null) {
				...
				if (idProperty != null) {
					...
					properties.computeIfAbsent(idProperty.value(), k -> fx_id);
				}
		...
		public void processCharacters() throws LoadException {
			...
			if (defaultProperty != null) {
				...
				if (valueAdapter.isReadOnly(defaultPropertyName) && List.class.isAssignableFrom(
						valueAdapter.getType(defaultPropertyName))) {
			...
			if (prefix != null && prefix.equals(FX_NAMESPACE_PREFIX)) {
				if (localName.equals(FX_ID_ATTRIBUTE)) {
					...
					if (current.parent != null) {
						throw constructLoadException(
								FX_NAMESPACE_PREFIX + ":" + FX_CONTROLLER_ATTRIBUTE + " can only be applied to root " +
										"element.");
					}
					...
					if (!staticLoad) {
						...
						if (controller != null) {
							if (!(controller.getClass().isAssignableFrom(type) || type.isAssignableFrom(
									controller.getClass()))) {
								throw constructLoadException(
										"Controller value already specified and types not assignable: %s, %s".formatted(
												controller.getClass(), type));
							}
						} else {
							try {
	...
	private class InstanceDeclarationElement extends ValueElement {
		...
		public void processAttribute(String prefix, String localName, String value) throws IOException {
			if (prefix != null && prefix.equals(FX_NAMESPACE_PREFIX)) {
				switch (localName) {
					case FX_VALUE_ATTRIBUTE -> this.value = value;
					case FX_CONSTANT_ATTRIBUTE -> constant = value;
					case FX_FACTORY_ATTRIBUTE -> factory = value;
					default -> super.processAttribute(prefix, localName, value);
				}
			} else {
				super.processAttribute(prefix, localName, value);
			}
	...
	private class UnknownTypeElement extends ValueElement {
		...
		public static class UnknownValueMap extends AbstractMap<String, Object> {
	...
	// Element representing an 'include' element
	private class IncludeElement extends ValueElement {
		public String source = null;
		public ResourceBundle resources = SpringFXLoader.this.resources;
		public Charset charset = SpringFXLoader.this.charset;
				...
				switch (localName) {
					...
					case INCLUDE_RESOURCES_ATTRIBUTE -> {
						...
						resources = ResourceBundle.getBundle(value, Locale.getDefault(),
								SpringFXLoader.this.resources.getClass().getClassLoader());
					}
		...
		public Object constructValue() throws IOException {
			...
			if (source.charAt(0) == '/') {
				...
				if (SpringFXLoader.this.location == null) {
					throw constructLoadException("Base location is undefined.");
				}

				location = new URL(SpringFXLoader.this.location, source);
			}

			SpringFXLoader fxmlLoader = new SpringFXLoader(
					location, resources, builderFactory, controllerFactory, charset, loaders, applicationContext);
			fxmlLoader.parentLoader = SpringFXLoader.this;

			if (isCyclic(SpringFXLoader.this, fxmlLoader)) {
				throw new IOException(String.format("Including \"%s\" in \"%s\" created cyclic reference.",
						fxmlLoader.location.toExternalForm(), SpringFXLoader.this.location.toExternalForm()));
			}
		...
		public void processStartElement() throws IOException {
			...
			if (source != null && !staticLoad) {
				...
				try {
					...
					if (source.charAt(0) == '/') {
						// FIXME: JIGSAW -- use Class.getResourceAsStream if resource is in a module
						location = Objects.requireNonNull(cl.getResource(source.substring(1)));
					} else {
						if (SpringFXLoader.this.location == null) {
							throw constructLoadException("Base location is undefined.");
						}
					...
					try {
						if (engine instanceof Compilable && compileScript) {
							...
							try {
								...
								Logging.getJavaFXLogger().warning(
										filename + ": compiling caused \"" + compileExc + "\", falling back to " +
												"evaluating script in uncompiled mode");
							}
						...
						log.error("%s: caused ScriptException".formatted(filename), exception);
					}
				} catch (IOException exception) {
					throw constructLoadException(exception);
				}
		...
		public void processEndElement() throws IOException {
			...
			if (value != null && !staticLoad) {
				...
				try {
					...
					filename =
							location.getPath() + "-script_starting_at_line_" + (getLineNumber() - (int) script.codePoints()
									.filter(c -> c == '\n')
									.count());
